...
async function dbCreatePlayer(username, password, playerData) {
  const { data, error } = await supabase.from("players").insert({ username, password_hash: password, player_data: playerData }).select().single();
  return { data, error };
}
async function dbSavePlayer(username, playerData) {
  await supabase.from("players").update({ player_data: playerData, updated_at: new Date().toISOString() }).eq("username", username);
}
async function dbGetAllPlayers() {
  const { data } = await supabase.from("players").select("player_data");
  return data ? data.map(r => r.player_data) : [];
}
async function dbGetSyndicates() {
  const { data } = await supabase.from("syndicates").select("data");
  return data ? data.map(r => r.data) : [];
}
async function dbSaveSyndicate(syndicate) {
  const existing = await supabase.from("syndicates").select("id").eq("name", syndicate.name).single();
  if (existing.data) {
    await supabase.from("syndicates").update({ data: syndicate }).eq("name", syndicate.name);
  } else {
    await supabase.from("syndicates").insert({ name: syndicate.name, data: syndicate });
  }
}
...
async function dbPostAnnouncement(text) {
...
function AuthPage({ onLogin }) {
  ...
  async function login() {
    ...
    if (!row) return setErr("Account not found.");
    if (row.password_hash !== form.password) return setErr("Wrong password.");
  ...
  async function register() {
    ...
    if (existing) return setErr("Username taken.");
    ...
    if (error) return setErr("Registration failed. Try again.");
    onLogin(player);
  }
...
function LeaderboardPage({ player }) {
  ...
  const [syndicates, setSyndicates] = useState([]);
  useEffect(()=>{
    dbGetAllPlayers().then(ps=>setPlayers(ps.sort((a,b)=>b.level-a.level||b.reputation-a.reputation)));
...
function Game({ initialPlayer, onLogout }) {
  ...
  useEffect(()=>{
    dbSavePlayer(player.username, player);
  },[player]);
