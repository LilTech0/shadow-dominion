...
const S={
  ...
  sb:{width:190,background:"#0b0b14",borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column"},
};
...
function AdminLogin({onLogin}){
  const [u,setU]=useState(""),[p,setP]=useState(""),err=useState("")[1];
...
function AdminPanel({onLogout}){
  ...
  async function refresh(){
    setLoading(true);
    const ps=await dbAllPlayers();
    setPlayers(ps);
    const as=await dbGetAnnouncements();
    setAnns(as);
    setLoading(false);
  }
  useEffect(()=>{refresh();},[]);
  ...
  async function delAnn(id){
    await dbDeleteAnnouncement(id);refresh();
  }
  ...
  return(<div style={{...S.app,background:"#09080a"}}>
    ...
    <div style={{background:"#0f0a00",borderBottom:`1px solid #2a1a00`,padding:"12px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <span style={{color:C.orange,fontSize:16,fontWeight:900,letterSpacing:3}}>⚙️ ADMIN PANEL — SHADOW DOMINION</span>
    ...
    <div style={{display:"flex",minHeight:"calc(100vh - 49px)"}}>
      <div style={{...S.sb,borderColor:"#2a1a00"}}>
        {[["players","👥","PLAYERS"],["give","🎁","GIVE ITEMS"],["edit","✏️","EDIT PLAYER"],["announce","📢","ANNOUNCE"]].map(([id,icon,label])=>(<div key={id} style={{...S.nav(tab===id),borderLeftColor:tab===id?C.orange:"transparent",color:tab===id?C.orange:C.muted}} onClick={()=>setTab(id)}><span>{icon}</span><span style={{fontSize:10}}>{label}</span></div>))}
      </div>
      <div style={{flex:1,padding:20,overflowY:"auto"}}>
        <div style={{display:"flex",gap:12,marginBottom:16,flexWrap:"wrap"}}>
          {[["👥 PLAYERS",players.length,C.blue],["💰 TOTAL CASH","$"+totalCash.toLocaleString(),C.green],["🔪 CRIMES",totalCrimes,C.orange]].map(([l,v,c])=>(<div key={l} style={{background:"#0f0a00",border:"1px solid #2a1a00",borderRadius:6,padding:"10px 16px",minWidth:110}}><div style={{color:c,fontWeight:900,fontSize:16}}>{v}</div><div style={{color:C.muted,fontSize:9}}>{l}</div></div>))}
        </div>
        ...
        {tab==="players"&&<div>
          <div style={S.card()}>
            ...
            {filtered.map(p=>(<div key={p.username} style={{display:"grid",gridTemplateColumns:"1fr 50px 60px 70px 80px",gap:6,padding:"8px 4px",borderBottom:`1px solid ${C.border}`,background:sel?.username===p.username?"#1a0e00":"transparent",cursor:"pointer",borderRadius:2}} onClick={()=>selectP(p)}>
        ...
        {tab==="give"&&<div>
          ...
          {sel&&<><div style={S.card()}>
            ...
            <div style={{display:"flex",gap:8}}><select style={{...S.inp,marginBottom:0,flex:1}} value={give.itemId} onChange={e=>setGive(g=>({...g,itemId:e.target.value}))}><option value="">Select item...</option>{ITEMS.map(i=><option key={i.id} value={i.id}>{i.name} ({i.rarity})</option>)}</select><button style={S.btn(C.orange,C.orangeBg)} onClick={giveItem}>GIVE ITEM</button></div>
          </div>
          <div style={S.card()}>
            <div style={{...S.ct,color:C.orange}}>🎒 INVENTORY</div>
            {sel.inventory?.length===0&&<div style={{color:C.dim}}>Empty.</div>}
            {sel.inventory?.map(id=>{const item=ITEMS.find(i=>i.id===id);return item?<div key={id} style={{color:C.text,fontSize:12,padding:"4px 0",borderBottom:`1px solid ${C.border}`}}>{item.name} <span style={S.badge(C.muted)}>{item.type}</span></div>:null;})}
          </div></>}
        </div>}
        ...
        {tab==="announce"&&<div>
          ...
          <div style={S.card()}>
            <div style={{...S.ct,color:C.orange}}>📋 ACTIVE ANNOUNCEMENTS</div>
            {anns.length===0&&<div style={{color:C.dim}}>None.</div>}
            {anns.map(a=>(<div key={a.id} style={{padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div><div style={{color:"#fff",fontSize:12,marginBottom:3}}>{a.text}</div><div style={{color:C.muted,fontSize:10}}>{new Date(a.created_at).toLocaleString()}</div></div>
                <button style={{...S.btn(C.red,C.redBg),padding:"3px 8px",fontSize:9,marginLeft:8}} onClick={()=>delAnn(a.id)}>DELETE</button>
              </div>
            </div>))}
          </div>
        </div>}
...
// MAIN GAME
const NAV=[{id:"profile",icon:"👤",label:"PROFILE"},{id:"crimes",icon:"🔪",label:"CRIMES"},{id:"combat",icon:"⚔️",label:"COMBAT"},{id:"inventory",icon:"🎒",label:"INVENTORY"},{id:"syndicates",icon:"🏴",label:"SYNDICATES"},{id:"leaderboard",icon:"🏆",label:"LEADERBOARD"}];
...
function Game({initialPlayer,onLogout}){
  ...
  return(<div style={S.app}>
    ...
    <div style={{display:"flex",minHeight:"100vh"}}>
      ...
      <div style={{flex:1,padding:20,overflowY:"auto",maxWidth:820}}>
